...
console.log('reports-app.js loaded, starting init...');
setFooterYear();
loadReports();
...
async function loadReports() {
  try {
    ...
    console.log('reports.json response status:', resp.status);
    ...
    console.log('Reports loaded, count =', allReports.length);
...
function handleFilterChange() {
  ...
  visibleReports = allReports.filter((r) => {
    ...
    // Prefer file_format but still tolerate legacy type
    const rFormat = r.file_format || r.type || '';
...
function renderReports() {
  ...
  const cardsHtml = visibleReports
    .map((r) => {
      ...
      // Prefer descriptive file_format, fallback to old "type" if present
      const format = r.file_format || r.type || '';
